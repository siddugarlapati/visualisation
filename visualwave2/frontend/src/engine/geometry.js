...
/**
 * Create a specialized geometry based on an architecture component type.
 * Options: 'frontend', 'backend', 'database', 'cache', 'queue', 'ml_model'
 * 
 * @param {string} type - Component type
 * @param {number} size - Base scale
 * @returns {THREE.BufferGeometry} 
 */
export function createIconGeometry(type = 'default', size = 0.5) {
    switch (type) {
        case 'frontend':
            // Thin screen / monitor display
            return new RoundedBoxGeometry(size * 1.6, size * 1.1, size * 0.1, 4, 0.05);

        case 'backend':
            // Server rack / blade
            return new RoundedBoxGeometry(size * 0.8, size * 1.5, size * 1.0, 4, 0.1);

        case 'database':
            // Classic cylinder
            return new THREE.CylinderGeometry(size * 0.7, size * 0.7, size * 1.4, 32);

        case 'cache':
            // Flattened cylinder / puck
            return new THREE.CylinderGeometry(size * 0.8, size * 0.8, size * 0.5, 32);

        case 'queue':
            // Horizontal pipe/tube
            const queueGeo = new THREE.CylinderGeometry(size * 0.5, size * 0.5, size * 1.5, 32);
            queueGeo.rotateZ(Math.PI / 2);
            return queueGeo;

        case 'ml_model':
            // Neural network brain / complex polyhedra
            return new THREE.IcosahedronGeometry(size, 1);

        default:
            // Generic rounded box
            return new RoundedBoxGeometry(size, size, size, 4, 0.15);
    }
}
